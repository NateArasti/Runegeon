...
public class PixelTexturePostProcessor : AssetPostprocessor
{
    ...
    private static PixelMap FindPixelMap(string fileName)
    {
        ...
        return guids
            .Select(guid => AssetDatabase.LoadMainAssetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as PixelMap)
            .FirstOrDefault(asset => asset != null && asset.name == fileName);
    }
}
